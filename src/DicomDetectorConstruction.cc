...
G4VPhysicalVolume* DicomDetectorConstruction::Construct()
{
  ...
  //
    ...
    G4Material* Polystyrene = nist->FindOrBuildMaterial("G4_POLYSTRENE");
  ...
  //  G4Material* detectorMaterial = G4NistManager::Instance()->FindOrBuildMaterial("G4_Water");
    ...
    G4Box* solidWater = new G4Box("Solid Water", 0.5 * CLHEP::cm, 10 * CLHEP::cm, 10 * CLHEP::cm);
    
    G4LogicalVolume* logicDetector = new G4LogicalVolume(solidWater, solid_water, "Solid Water");
    
    auto physDetectoor = new G4PVPlacement(nullptr, G4ThreeVector(0, 0, 0), logicDetector, "Solid Water", fWorld_logic, false, 0, false);
  ...
  //
    ...
    G4Tubs* outer_chamber = new G4Tubs("OuterChamber", 0., outer_radius, chamber_length / 2., 0., 360.0 * CLHEP::deg);
    G4LogicalVolume* logic_outer_chamber = new G4LogicalVolume(outer_chamber, steel, "OuterChamber");

    // Внутренний цилиндр (внутренний электрод)
    G4Tubs* inner_chamber = new G4Tubs("InnerChamber", 0., inner_radius, chamber_length / 2., 0., 360.0 * CLHEP::deg);
    G4LogicalVolume* logic_inner_chamber = new G4LogicalVolume(inner_chamber, steel, "InnerChamber");

    // Создание физического расположения (внешний цилиндр)
    new G4PVPlacement(0, G4ThreeVector(), logic_outer_chamber, "OuterChamber", 0, false, 0);
